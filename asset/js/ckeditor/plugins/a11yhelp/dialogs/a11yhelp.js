...
CKEDITOR.dialog.add("a11yHelp", function (editor) {
    // Function to map keyboard modifiers to their respective labels
    function mapModifiersToLabels(modifier) {
        return g.map(function (key) {
            const label = labels[key] || String.fromCharCode(key);
            return modifier / key > 1 && modifier / key <= 2 ? label : '';
        }).join('+');
    }

    // Function to get keyboard shortcut for a command
    function getCommandShortcut(command) {
        const keystrokes = editor.getCommandKeystroke(command, true);
        return keystrokes.length ? CKEDITOR.tools.array.map(keystrokes, mapModifiersToLabels).join(' / ') : a.title;
    }

    const a = editor.lang.a11yhelp;
    const b = editor.lang.common.keyboard;
    const p = CKEDITOR.tools.getNextId();
    const q = /\$\{(.*?)\}/g;
    const g = [CKEDITOR.ALT, CKEDITOR.SHIFT, CKEDITOR.CTRL];
    const labels = {
        // Keyboard modifier labels
        [CKEDITOR.ALT]: b[18],
        [CKEDITOR.SHIFT]: b[16],
        [CKEDITOR.CTRL]: CKEDITOR.env.mac ? b[224] : b[17]
    };
            ...
            elements: [{
                ...
                html: function () {
                    ...
                    function replacePlaceholders(str, placeholders) {
                        return str.replace(q, function (match, placeholder) {
                            return placeholders[placeholder] || match;
                        });
                    }

                    const legendsHTML = a.legend.map(function (legendItem) {
                        const itemsHTML = legendItem.items.map(function (item) {
                            const legendEdge = CKEDITOR.env.edge && item.legendEdge ? item.legendEdge : item.legend;
                            const legendText = replacePlaceholders(legendEdge, { shortcut: getCommandShortcut(item.command) });
                            return `<dt>${item.name}</dt><dd>${legendText}</dd>`;
                        }).join('');

                        return `<h1>${legendItem.name}</h1><dl>${itemsHTML}</dl>`;
                    }).join('');

                    return `<div class="cke_accessibility_legend" role="document" aria-labelledby="${p}_arialbl" tabIndex="-1">${legendsHTML}</div>
                            <span id="${p}_arialbl" class="cke_voice_label">${a.contents}</span>`;
                }
            }]
        }],
        buttons: [CKEDITOR.dialog.cancelButton]
